...
@section('body')
    <div class="alert alert-dark" role="alert">
        <span>{{__('words.Messages')}}</span>
    </div>
    <form class="row g-3" action="{{route('dashboard.messages.store')}}" method="POST">
        @csrf
        <div class="col-12">
            <label for="inputEmail4" class="form-label">{{__('words.Email')}}</label>
            <input type="email" class="form-control" placeholder="Example@email.com" id="inputEmail4">
        </div>
        <div class="col-6">
